# Ancienne implementation conservee ci-dessous en commentaire de reference
# pour ne pas detruire le travail precedent.
...
# Section modifiee: chargement du nouveau vectorizer DBSCAN-compatible.
@lru_cache(maxsize=1)
def get_vectorizer():
    return joblib.load(VECTORIZER_PATH)


# Section ajoutee: chargement mis en cache du modele DBSCAN.
@lru_cache(maxsize=1)
def get_dbscan():
...
# Section ajoutee: vectorisation du CV avec transform() pour reutiliser le modele entraine.
...
# Section ajoutee: normalisation pour apparier les offres Django avec le CSV DBSCAN.
...
# Section ajoutee: preparation des vecteurs offres pour le clustering DBSCAN.
# Si tfidf_vector manque en base, on le recalcule a la volee avec le meme vectorizer.
...
# Section ajoutee: attribution d'un cluster au CV par plus proches voisins,
